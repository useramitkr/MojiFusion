...
export default function Tile({ value, row, col }: Props) {
  ...
  // Get available emojis for switching (same value or lower)
  const availableEmojis = Object.entries(themes[theme])
    .filter(([key, _]) => Number(key) <= value)
    .map(([key, emoji]) => ({ value: Number(key), emoji }));

  const handleLongPress = () => {
    console.log("Long press detected, switcher count:", switcherCount);
    if (switcherCount > 0) {
  ...
  const handleSwitch = (newValue: number) => {
    console.log("Switching tile from", value, "to", newValue);
    switchTile(row, col, newValue);
      ...
      >
        ...
        >
          <View style={styles.switcherModal}>
            <Text style={styles.modalTitle}>🎯 Choose New Emoji</Text>
            <Text style={styles.modalSubtitle}>
              Switchers left: {switcherCount} 🔄
            </Text>
            ...
            >
              {availableEmojis.map(({ value: emojiValue, emoji: emojiChar }) => (
                <TouchableOpacity
                  key={emojiValue}
                  style={[
                    styles.emojiOption,
                    {
                      backgroundColor: getTileStyle(emojiValue).bg,
                      borderColor: getTileStyle(emojiValue).border,
                    },
                    emojiValue === value && styles.currentEmoji
                  ]}
                  onPress={() => handleSwitch(emojiValue)}
                >
                  <Text style={styles.emojiText}>{emojiChar}</Text>
                  <Text style={styles.emojiLabel}>{emojiValue}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
